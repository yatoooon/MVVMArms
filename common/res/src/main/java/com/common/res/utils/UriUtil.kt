 ...
 */
class UriUtil @JvmOverloads constructor(url: String?, charset: String? = null) {
     ...
     * @return 返回当前类
     */
    fun addPath(pathAdded: String?, encode: Boolean): UriUtil {
     ...
     * @return 返回当前类
     */
    fun addQuery(name: String?, value: String?, encode: Boolean): UriUtil {
     ...
     * @return 返回当前类
     */
    fun addQueries(input: HashMap<String, String>?, encode: Boolean): UriUtil {
     ...
     * @return 返回当前类
     */
    fun appendQuery(name: String?, value: String?, encode: Boolean): UriUtil {
        if (name.isNullOrBlank() || value.isNullOrEmpty()) {
            return this
        }
        if (encode) {
     ...
     * @return 返回当前类
     */
    fun appendQueries(input: HashMap<String, String>?, encode: Boolean): UriUtil {
        if (input == null || input.size == 0) {
            return this
        }
        if (encode) {
     ...
     * @return 返回当前类
     */
    fun removeQuery(name: String?): UriUtil {
        if (name == null) {
            return this
        }
    ...
    companion object {
        ...
        fun from(url: String?, charset: String? = null): UriUtil {
            return UriUtil(url, charset)
        }
    }
}
