...
import com.basis.common.layout.SimpleLayout;
...
public final class PlayerView extends SimpleLayout
    ...
    public boolean onInfo(MediaPlayer player, int what, int extra) {
        switch (what) {
            ...
            case MediaPlayer.MEDIA_INFO_BUFFERING_START:
                ...
                mMessageView.setText(R.string.res_common_loading);
    ...
    public boolean onError(MediaPlayer player, int what, int extra) {
        ...
        if (what == MediaPlayer.MEDIA_ERROR_NOT_VALID_FOR_PROGRESSIVE_PLAYBACK) {
            message = activity.getString(R.string.res_common_video_error_not_support);
        } else {
            message = activity.getString(R.string.res_common_video_error_unknown);
        }
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mTouchOrientation == LinearLayout.HORIZONTAL) {
                    ...
                    if (progress >= 0 && progress <= getDuration()) {
                        ...
                        mLottieView.setImageResource(mAdjustSecond < 0 ?
                                R.drawable.res_video_schedule_rewind_ic :
                                R.drawable.res_video_schedule_forward_ic);
                ...
                if (mTouchOrientation == LinearLayout.VERTICAL) {
                    ...
                    if ((int) event.getX() < getWidth() / 2) {
                        ...
                        if (percent > 100 / 3 * 2) {
                            iconId = R.drawable.res_video_brightness_high_ic;
                        } else if (percent > 100 / 3) {
                            iconId = R.drawable.res_video_brightness_medium_ic;
                        } else {
                            iconId = R.drawable.res_video_brightness_low_ic;
                        }
                    ...
                    @DrawableRes int iconId;
                    if (percent > 100 / 3 * 2) {
                        iconId = R.drawable.res_video_volume_high_ic;
                    } else if (percent > 100 / 3) {
                        iconId = R.drawable.res_video_volume_medium_ic;
                    } else if (percent != 0) {
                        iconId = R.drawable.res_video_volume_low_ic;
                    } else {
                        iconId = R.drawable.res_video_volume_mute_ic;
                    }
                    mLottieView.setImageResource(iconId);
                    mMessageView.setText(String.format("%s %%", percent));
                    post(mShowMessageRunnable);
